...
/**
 * Counts occurences of given street name in given voivodeship
 * @author Michał Śliwa
 */
        ...
        try
        {
            ...
            DefaultHandler handler = new DefaultHandler()
            {
                        ...
                        throws SAXException
                {
                    ...
                    if (inWoj)
                        if(new String(ch,start,length).equals(wojCode))
                            //set isWojOk to true
                            isWojOk = true;
                    //if in NAZWA_1 tag and WOJ tag content matches, check if
                    //content contains street name
                    if (isWojOk && inNazwa)
                        ...
                        String qName) throws SAXException
                {
                    ...
                    if (qName.equalsIgnoreCase("WOJ"))
                        inWoj = false;
                    //if tag name is NAZWA_1 set inNazwa to flase
                    if (qName.equalsIgnoreCase("NAZWA_1"))
                        inNazwa = false;
                    //if tag name is row reset WojName check
